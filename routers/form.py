...
async def list_view(request: Request, db: AsyncSession = Depends(get_db), current_user: AuthUser = Depends(get_current_user)):
    ...
    async def get_full_form_dict(form_id, form_obj=None):
        ...
        form_dict = {
            ...
            "flocation_type": {"id": loc_o.id, "name": loc_o.l_location, "l_location": loc_o.l_location} if loc_o else None,
            "flocation_description": form_obj.flocation_description,
            "fjuridiction": {"id": jur_o.id, "name": jur_o.l_juridiction, "l_juridiction": jur_o.l_juridiction} if jur_o else None,
            "fincident": {"id": inc_o.id, "name": inc_o.i_incident, "i_incident": inc_o.i_incident} if inc_o else None,
            "fweight_data": {"id": wei_o.id, "name": wei_o.w_weight, "weight": wei_o.w_weight} if wei_o else None,
            "fexplosive": {"id": exp_o.id, "name": exp_o.e_explosive, "explosive_name": exp_o.e_explosive} if exp_o else None,
            ...
            "fassume_status_new": {"id": ass_o.id, "name": ass_o.a_assused, "a_assused": ass_o.a_assused} if ass_o else None,
            "mode_of_detection": {"id": det_o.id, "name": det_o.d_ditection, "detection_name": det_o.d_ditection} if det_o else None,
            ...
            "detected_dispose": {"id": dis_o.id, "name": dis_o.d_dispose, "dispose_type": dis_o.d_dispose} if dis_o else None,
    ...
    for form, loc_obj in rows:
        ...
        form_list.append({
            ...
            "flocation_type": {
                ...
                "name": loc_obj.l_location if loc_obj else None,
